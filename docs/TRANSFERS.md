...
detail workflow checks `Artwork.downloadAvailability` before resolving an original and
...
video, animation, archive, document, or downloadable text assets. Rendered
literature and journals are fetched through `ArtworkContentRepository`; they are
content data, not invented files. Actual provider access still depends on whether
an item is downloadable, the account's authorization, purchase/maturity
restrictions, and the original metadata endpoint.
