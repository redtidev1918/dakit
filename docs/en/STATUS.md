...
- Version stage: `dakit_core` 0.1.6, `dakit_api` 0.1.8, and `dakit_flutter` 0.1.4, all published to pub.dev;
- Runtime: Flutter 3.47.1 / Dart 3.13.1;
- Platforms: Android, macOS, Windows/MSIX;
- Tests: formatting and analysis pass, 101 tests pass;
...
- Package publishing: `dakit_core` 0.1.6, `dakit_api` 0.1.8, and `dakit_flutter` 0.1.4 have been successfully uploaded to pub.dev (verified via both paths: GitHub Actions OIDC automated publishing and local manual publishing);
...
Full CI covers the Linux quality gate (analyze / format / test) and the Android APK, macOS app, and Windows/MSIX builds. The quality gate previously failed because the example_client test fakes did not implement `TransferManager.moveToSharedStorage` added in `dakit_core` 0.1.5; those fakes have been filled in, and all 101 tests pass locally.
