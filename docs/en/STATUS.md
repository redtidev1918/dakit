...
- Tests: formatting and analysis pass, 109 tests pass;
...
Full CI covers the Linux quality gate (analyze / format / test) and the Android APK, macOS app, and Windows/MSIX builds. The quality gate previously failed because the example_client test fakes did not implement `TransferManager.moveToSharedStorage` added in `dakit_core` 0.1.5; those fakes have been filled in, and all 109 tests pass locally.
