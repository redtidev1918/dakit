...
- Version stage: `dakit_core` 0.1.14, `dakit_api` 0.1.28, and `dakit_flutter` 0.1.11, all published to pub.dev;
...
- Package publishing: `dakit_core` 0.1.14, `dakit_api` 0.1.28, and `dakit_flutter` 0.1.11 have been successfully uploaded to pub.dev (verified via both paths: GitHub Actions OIDC automated publishing and local manual publishing);
...
Full CI covers the Linux quality gate (analyze / format / test) and the Android APK, macOS app, and Windows/MSIX builds; the CLI tag pipeline separately covers five native binaries. All 135 tests pass locally.
