...
Future<Map<String, Object?>> _verify(_Configuration configuration) async {
  ...
  final transport = OfficialApiClient(
    ...
    config: ApiConfig(userAgent: 'DAKit-LiveVerifier/0.1'),
  );
  ...
  final mediaClient = createNetworkDio(
    ...
    options: BaseOptions(
      ...
      headers: const <String, Object?>{
        'User-Agent': 'DAKit-LiveVerifier/0.1',
...
Future<Map<String, Object?>> _verifyCase(
  ...
  try {
    ...
    final expectedRestriction =
        ...
        (error.kind == DAKitFailureKind.authentication ||
            error.kind == DAKitFailureKind.authorization ||
            error.kind == DAKitFailureKind.restricted);
    return <String, Object?>{
...
final class _Configuration {
  ...
  factory _Configuration.parse(
    ...
    final token = environment['DAKIT_ACCESS_TOKEN']?.trim();
    if (token == null || token.isEmpty) {
      throw const _UsageException('DAKIT_ACCESS_TOKEN is required.');
    }
    ...
    final output = environment['DAKIT_LIVE_OUTPUT']?.trim();
    final outputDirectory = output == null || output.isEmpty
        ? Directory(
            '${Directory.systemTemp.path}${Platform.pathSeparator}'
            'dakit-live-${DateTime.now().toUtc().microsecondsSinceEpoch}',
          )
        : Directory(output);
...
const _usage = '''
Usage:
  DAKIT_ACCESS_TOKEN=... dart run example/live_contract.dart \\
...
Optional environment:
  DAKIT_LIVE_OUTPUT=/safe/output/directory
