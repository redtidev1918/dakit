...
Future<int> _url(ArgResults arguments) async {
  final uuid = _extractUuid(arguments.rest.firstOrNull);
  if (uuid == null) {
...
Future<int> _downloadBatch(
  ...
  final limit = _positiveInt(arguments['limit'] as String, 24, '--limit');
  final delay = _positiveInt(arguments['delay'] as String, 1, '--delay');
  ...
  final outputDirectory = organize == 'flat'
      ? root
      : Directory(
          '${root.path}${Platform.pathSeparator}${_safeFilename(destination)}',
        );
  await outputDirectory.create(recursive: true);
...
Future<_CliContext?> _session(ArgResults arguments) async {
  final tokens = await FileTokenStore().read();
  if (tokens == null) {
    stderr.writeln('Not logged in. Run `dakit login` first.');
    return null;
  ...
  return _CliContext(
    profile: _profile(arguments['proxy'] as String?),
    diagnostics: _diagnostics(arguments),
    tokens: tokens,
  );
}

String? _extractUuid(String? value) {
...
int _positiveInt(String value, int fallback, String label) {
