...
NetworkProfile readExampleNetworkProfile() => parseExampleNetworkProfile(
  ...
  host: const String.fromEnvironment('DAKIT_PROXY_HOST'),
  port: const String.fromEnvironment('DAKIT_PROXY_PORT'),
  bypass: const String.fromEnvironment('DAKIT_PROXY_BYPASS'),
);

ProxyConfiguration? readExampleTransferProxy() => parseExampleTransferProxy(
  host: const String.fromEnvironment('DAKIT_TRANSFER_PROXY_HOST'),
  port: const String.fromEnvironment('DAKIT_TRANSFER_PROXY_PORT'),
);
...
NetworkProfile parseExampleNetworkProfile({
  ...
  _ => throw const DAKitException(
    kind: DAKitFailureKind.configuration,
    code: 'example.proxy.mode_invalid',
    message: 'DAKIT_PROXY_MODE must be environment, direct, or http.',
  ),
};
...
int _proxyPort(String value) {
  ...
  if (parsed == null) {
    throw const DAKitException(
      ...
      message: 'DAKIT_PROXY_PORT must be an integer.',
    );
  }
  return parsed;
}
